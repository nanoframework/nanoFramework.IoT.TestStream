...
public class Runner
{
    ...
    /// </summary>
    public static int ReturnValue { get; set; } = 0;
    ...
    private static void CreateSetup()
    {
        ...
        if (!isInstalled)
        {
            var res = MessageBox.Query("WSL is not installed. Do you want to install WSL2 before continuing with Docker and all the needed elements?", "Information", "Yes", "No");
            if (res == 0)
            {
                var install = ProcessHelpers.RunCommand("powershell.exe",
                    $"-NoProfile -ExecutionPolicy Bypass -File \"{Path.Combine(AppContext.BaseDirectory, "agent", "install.ps1")}\" -WSLDistribution {OverallConfiguration.Config.WslDistribution}",
                    outputConsole: true,
                    useShell: true);
            }
        }
